...
`🎬 流媒体自动` 是同一批接入点的 `url-test` 版，嫌手动切麻烦就选它。
...
QoS 设备很难区别对待）。可选：Zero Trust 控制台里把 MASQUE 降级成
HTTP/2（HTTP/2 跑在 TCP 上），或自备一个 VLESS/Trojan over TCP+TLS 出口
（接法同下面 AI 那段：加节点 + `dialer-proxy` 指向 MASQUE 接入点）。
仓库本身只生成 MASQUE 配置，这条得在客户端和控制台做。
...
- **不共用**：接入点。免费边缘 57 个走免费密钥，团队边缘 4 个走团队密钥
...
254 项，覆盖常数时间比较、token 伪造/篡改/过期、登录限速、并发初始化，
...
自动选择不再嵌套引用分组、QUIC 规则排在最前、敏感域名规则排在 RULE-SET 之前、
流媒体 CDN
