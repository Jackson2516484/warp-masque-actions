...
600 项，覆盖常数时间比较、token 伪造/篡改/过期、登录限速、并发初始化，
...
最后一份（`worker/test/dist.test.mjs`）测的不是源码，是**产物**：
它把 `dist/worker.js` 整个载进来，直接调里面的 `buildConfig`，
把上面那些关键不变量再验一遍（探测点分档、粘性、兜底链、
YouTube 整族同出口、DNS 钉死），共 33 项。
多加这一层是因为：源码测试全绿，也可能因为忘了 `npm run build`
而把**旧产物**部署上去。它还会在 `src/` 的文件比 `dist/worker.js`
新的时候给一句警告。
