...
电脑上却正常**」。默认出口换成 `url-test` 组后就不吃这个兼容性坑了，想用聚合
手选一下即可。
...
1. **切 `🎬 流媒体` 组**。YouTube 的规则目标就是这个组。默认走「🎬 流媒体自动」，
   还不行就往下切成 **Proton线路 / Windscribe线路** —— CF 自己的出口 IP 被 Google
   判成机房限流时，只有换出口能救。
...
**Argo 智能选路**（CF 自己的优质骨干），并且**免费边缘和 ZT 团队边缘都吃这个账号**，
绑一次全族受益。
...
想验证有没有生效：管理页点「设备体检」，结果里会带上 `warp_plus` 和剩余额度；
或者连上之后访问 `https://www.cloudflare.com/cdn-cgi/trace`，
...
| `/api/warp/license` | POST，绑 WARP+ 授权码。`{key, fresh}`；`fresh: true` 换新设备再绑 |
...
481 项，覆盖常数时间比较、token 伪造/篡改/过期、登录限速、并发初始化，
